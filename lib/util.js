...
util.validateConfig = conf => {
    ...
    if (
        ...
        (!_.isInteger(maxBlacklistPerUnit) || maxBlacklistPerUnit <= 0)
    )
        throw new Error("maxBlacklistPerUnit should be an unsigned integer");
    if (!_.isUndefined(error) && (!_.isNumber(error) || error <= 0))
        throw new Error("error should be a positive number");
    if (
        !_.isUndefined(expiresDuration) &&
        (!_.isInteger(expiresDuration) || expiresDuration <= 0)
    )
        throw new Error("expiresDuration should be an unsigned integer");
};
